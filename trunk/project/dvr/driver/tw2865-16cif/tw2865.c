...
static void tw2865_video_init(unsigned char addr, unsigned char mode)
{
    ...
    for (i=0; i<4; i++)
    {
        tw2865_byte_write(addr, 0x01 + 0x10*i, 0x08);/* brightness. */
        tw2865_byte_write(addr, 0x02 + 0x10*i, 0x50);/* luminance contrast gain */
        tw2865_byte_write(addr, 0x03 + 0x10*i, 0xb9);/* sharpness */
        tw2865_byte_write(addr, 0x04 + 0x10*i, 0x70);/* Chroma (U) */
        tw2865_byte_write(addr, 0x05 + 0x10*i, 0x70);/* Chroma (V) */
