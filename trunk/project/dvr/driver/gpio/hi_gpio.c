...
static int __init hi_gpio_init(void)
{
	...
	//add by lyh 2010.03.08 更改GPIO复用关系为IO输入
	WRITE_REG(IOCONFIG(9), 1);//buzzer

	WRITE_REG(IOCONFIG(30), 0);//alarm out
#ifdef NO_USE_74HC165
	WRITE_REG(IOCONFIG(25), 1);//senser in 0
	WRITE_REG(IOCONFIG(31), 0);//senser in 1
	WRITE_REG(IOCONFIG(33), 0);//senser in 2
	WRITE_REG(IOCONFIG(32), 0);//senser in 3
#endif
